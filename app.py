...
def speak_color_name(name, rate, pitch):
    engine = pyttsx3.init()
    engine.setProperty('rate', rate)
    voices = engine.getProperty('voices')
    engine.setProperty('voice', voices[0].id)  # Default voice
    engine.say(f"The color is {name}")
    engine.runAndWait()
...
voice_speed = st.sidebar.slider("🔊 Voice Speed", min_value=100, max_value=300, value=150, step=10)
voice_pitch = st.sidebar.slider("🎚️ Voice Pitch", min_value=50, max_value=200, value=100, step=10)

st.sidebar.markdown("✅ Built with `OpenCV`, `PIL`, `Streamlit`, `pyttsx3`")
...
if uploaded_file:
    ...
    with col2:
        if coords:
            ...
            # Store in detected history
            st.session_state.setdefault("color_history", [])
            st.session_state.color_history.append({
                "Color Name": match["color_name"],
                "R": R,
                "G": G,
                "B": B,
                "HEX": match["hex"]
            ...
            if st.button("🔊 Speak Color Name"):
                speak_color_name(match['color_name'], voice_speed, voice_pitch)
        else:
...
st.markdown("<div class='footer'>Developed by Srinithija 💖 | Enhanced with Voice, History & Styling 🌟</div>", unsafe_allow_html=True)
